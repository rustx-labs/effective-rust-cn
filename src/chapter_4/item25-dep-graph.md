...
更加令人担忧的是供应链攻击。别有用心的攻击者会通过[误植域名][typo squatting]、[劫持维护者账号][hijack account]或者其他更加精巧且隐蔽的方式，来破坏被广泛使用的公共依赖项，从而达到攻击的目的。

千万不要以为攻击行为仅在运行代码的时候才会发生，在*编译*期间，依赖项也可以通过 [`build.rs`][build.rs] 或者过程宏执行任意代码，这就意味着被破坏的依赖项可以在你的 CI 系统中偷偷运行挖矿程序！
...
[^1]: 除 [crates.io] 之外，也可以配置[备用注册中心][alternate registry]（例如公司内部的注册中心）。*Cargo.toml* 中的每个依赖项都可以设置 `registry` 键来表明其来源注册中心。
...
[hijack account]: https://eslint.org/blog/2018/07/postmortem-for-malicious-package-publishes/
