...
使用 newtype 模式（[第 6 条]）来包装一个 `bool` 也可以实现类型安全和可维护性；如果语义始终是布尔型的，通常最好使用这种方式，如果将来可能会出现新的选择（例如 `Sides::BothAlternateOrientation`），则应使用枚举类型。
...
使用标准类型使得设计意图更清晰，并且允许使用标准转换（[第 3 条]）和错误处理（[第 4 条]）；它还使得使用 `?` 运算符来简化错误处理成为可能。
