...
在撰写本文时（截至Rust 1.70版本），没有办法从 `Shape` 向上转型到 `Draw`，因为无法在运行时恢复（纯净的）`Draw` 虚表；没有办法在相关的 trait 对象之间进行转换，这也就意味着没有 [Liskov 替换原则]。然而，这种情况很可能在 Rust 的后续版本中发生变化 —— 有关更多信息，请参见[第 19 条]。
...
[Liskov 替换原则]: https://en.wikipedia.org/wiki/Liskov_substitution_principle
